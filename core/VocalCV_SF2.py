...
SF2_PATH = r"D:\AirPiano\Full Grand Piano.sf2"
...
try:
    ...
    print(f"Internal Synth Error: {e}")
...
while cap.isOpened():
    ...
    cv2.rectangle(frame, (20, 45), (20 + vol_w, 50), (255, 255, 0), -1)
    
    cv2.putText(frame, f"OCT: {cur_oct//12}", (w-120, 35), 0, 0.7, (255, 255, 0), 2)
    if current_right_finger != -1:
        cv2.putText(frame, f"CHORD: {current_chord_label}", (20, 75), 0, 0.8, (255, 255, 255), 2)
                ...
                if current_right_finger != -1:
                    ...
                    for i, t_id in enumerate(tips):
                        ...
                        if np.sqrt((thumb.x - tip.x)**2 + (thumb.y - tip.y)**2) < 0.04:
                            ...
                            if sf2:
                                for n in active_right_notes:
                                    sf2.play_note(midi_to_name(n), 100)
    ...
    cv2.imshow('Musicpy Air Piano Pro', frame)
